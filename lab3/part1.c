...
int num_vending_machines;
extern "C" void sim()		// main process
{
	printf("Enter the number of vending machines: ");
	scanf("%d", &num_vending_machines);
	vending_machine = new facility_set("vending machine", num_vending_machines);
...
void dep_cust()
{
	...
	long min_qlength = (*vending_machine)[0].qlength();
	int min_qlength_index = 0;
	for (int i=0; i<num_vending_machines; i++) {
		if ( (*vending_machine)[i].qlength() < min_qlength ) {
			min_qlength = (*vending_machine)[i].qlength();
			min_qlength_index = i;
		}
	}
	long Q = min_qlength; 
	bool stay = false;
	if (uniform(0,1) < (40+B)/(25*(3+Q)) ) { stay = true; }
	if (stay) {
		(*vending_machine)[min_qlength_index].reserve();
		hold(uniform(0,150+normal(B/2,30)));
		(*vending_machine)[min_qlength_index].release();
	}
	/*
